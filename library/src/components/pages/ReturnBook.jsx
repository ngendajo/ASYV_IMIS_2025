  ...
  return (
    <center className='formelement'>
      ...
      {(Array.isArray(data))? 
          data.map((borr,index)=>(
            <div key={index}>
                <p><strong>Student: </strong>{borr.first_name} {borr.rwandan_name}</p>
                <p><strong>From</strong> {borr.grade_name} <strong>Grade</strong> {borr.family_name} <strong>Family</strong> {borr.combination_name} <strong>Class</strong></p>
                <p><strong>Book returned: </strong>{borr.book_name} {borr.author_name} <strong>Author, from</strong> {borr.category_name} <strong>Category</strong></p>
                ...
                <label htmlFor="loginbutton">
                    <button onClick={(e) => confirmreturn(borr.issue_id)} className='submitbuton'>Save Return a Book</button> 
                </label>:
